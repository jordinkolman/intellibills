...
* **Stack:** Go (Backend API) + PostgreSQL 16 (Database).
* **Pattern:** Thin Database Architecture 
* **Execution:** The Go application executes standard `SELECT`, `INSERT`, `UPDATE`, and `DELETE` queries directly against base tables. The database engine is strictly responsible for enforcing data integrity, tenant isolation, and audit logging.
...
* **`app_runtime`**: The identity used by the Go API for standard user requests. It is restricted by Row Level Security (RLS) and relies on transaction-scoped session variables to access data. Write access to global lookup tables is explicitly revoked.
...
* **Global Tables:** Lookup tables (e.g., `auth.hash_algo`, `finance.account_type`, `finance.account_subtype`) are global. The Go API cannot write to them. They are populated strictly via migration seed scripts or the `app_worker`. User-defined categories (e.g., `budget.user_category`) remain writable by `app_runtime`.

## 5. The Go API Contract
To prevent connection pooling leaks and correctly trigger RLS and audit logs, every authenticated database interaction by `app_runtime` MUST be wrapped in a single database transaction (`db.BeginTx`).

Before executing any business logic query, the repository must inject the context:
```go
_, err = tx.ExecContext(ctx, "SET LOCAL app.context.current_tenant_id = $1; SET LOCAL app.context.request_id = $2;", userID, requestID)
```
...
Ephemeral Container Testing (testcontainers-go): The Go test suite programmatically spins up isolated PostgreSQL instances, runs migrations, executes integration tests, and destroys the container.

Application-Layer Integration: Go tests utilize the Transaction Rollback Pattern (executing queries inside a transaction and calling tx.Rollback() instead of tx.Commit()) to ensure test isolation and database cleanliness.
...
Test Seed (Dynamic): Handled dynamically by the Go test suite at runtime to ensure precise, deterministic setups for each test case.
